...
extension MQTTDecoder {
    ...
    func decodePUBREC(remainingData: Data) -> MQTTPUBREC? {
        ...
        guard let reasonCode = MQTTPUBRECReasonCode(rawValue: remainingData[pointer]) else {
            return nil
        }
        pointer += 1
        ...
        let packet = MQTTPUBREC(packetIdentifier: packetIdentifier, reasonCode: reasonCode)
