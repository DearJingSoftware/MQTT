...
extension MQTTDecoder {
    func decodeUNSUBACK(remainingData: Data) -> MQTTUNSUBACK? {
        ...
        if pointer < totalCount {
            while pointer < totalCount {
                guard let reasonCode = MQTTUNSUBACKReasonCode(rawValue: remainingData[pointer]) else {
                    return nil
                }
        ...
        let packet = MQTTUNSUBACK(packetIdentifier: packetIdentifier)
