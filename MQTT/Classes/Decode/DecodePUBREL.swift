...
extension MQTTDecoder {
    func decodePUBREL(remainingData: Data) -> MQTTPUBREL? {
        ...
        guard let reasonCode = MQTTPUBRELReasonCode(rawValue: remainingData[pointer]) else {
            return nil
        }
        pointer += 1
        ...
        let packet = MQTTPUBREL(packetIdentifier: packetIdentifier, reasonCode: reasonCode)
